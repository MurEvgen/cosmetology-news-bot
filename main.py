...
3. Основная часть (3-5 предложений): ключевые факты, цифры, детали исследования или методики.
4. Практическая польза (1-2 предложения): что это значит для врачей/пациентов
5. В конце: 🔗 <a href="{news_item['link']}">Читать в источнике</a>
6. Добавь 2-3 хэштега (#косметология #медицина #исследования)

ВАЖНО:
- Объем: 4-8 предложений (идеальный баланс детализации и читаемости).
- Живой язык, без канцеляризмов.
- Добавь 1-2 тематических эмодзи (🔬 💉 🧬 🌿 📊).
- Сохрани научную точность, не выдумывай факты.
...
ОТВЕТЬ ТОЛЬКО готовым HTML-текстом поста. Без комментариев."""
    try:
        resp = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Ты медицинский редактор. Пиши экспертные посты. Строго следуй формату HTML."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.6,
            max_tokens=800  # ✅ Безопасный лимит, не превышающий 1000 OTPM
        )
...
3. Раскрой тему глубоко: 4-8 предложений. Добавь контекст, объясни термины
...
ВАЖНО:
- НЕ упоминай источник, НЕ добавляй ссылки на оригинал
...
ОТВЕТЬ ТОЛЬКО готовым HTML-текстом поста или словом SKIP."""
    try:
        resp = groq_client.chat.completions.create(
            model="qwen/qwen3.8-27b",
            messages=[
                {"role": "system", "content": "Ты редактор. Пиши экспертные посты. Отвечай ТОЛЬКО текстом поста или словом SKIP."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            max_tokens=800  # ✅ Безопасный лимит
        )
